"""Train any RoboTwin-* VLA-RL task.

Usage:
    # GRPO on place_empty_cup (canonical)
    python scripts/vla/rl/train_robotwin.py \
        --task RoboTwin-PlaceCup-GRPO-v0 --num_envs 4 --max_iterations 10

    # PPO variant
    python scripts/vla/rl/train_robotwin.py \
        --task RoboTwin-PlaceCup-PPO-v0  --num_envs 4 --max_iterations 10

The script:
  1. Initializes engines that need to load before torch (sapien/Vulkan).
  2. Imports ``RRF_robotwin_vla_rl_tasks`` to register all RoboTwin task IDs.
...
import RRF_robotwin_vla_rl_tasks  # noqa: F401
