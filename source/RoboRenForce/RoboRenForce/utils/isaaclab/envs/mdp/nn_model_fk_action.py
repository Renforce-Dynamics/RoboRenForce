...
if TYPE_CHECKING:
    ...
    from RoboRenForce.utils.env_wrapper.lab_wrapper.nn_model.imagine_env_wrapper import RFImagineEnvWrapper
    from RoboRenForce.components.nn_models.system_dynamics.system_dynamics_mlp import SystemDynamicsMLP


class NNModelFKAction(ActionTerm):
    """Action term that uses a learned system dynamics nn model to drive FK state.
    ...
    - On (decimation) ticks, it rolls out the nn model to predict the next dynamic state.
    ...
    cfg: "NNModelFKActionCfg"

    def __init__(self, cfg: "NNModelFKActionCfg", env: "ManagerBasedRLEnv") -> None:
    ...
    def action_dim(self) -> int:
        ...
        configured in :class:`NNModelFKActionCfg`.
        """
    ...
    def set_env_wrapper(self, env_wrapper: "RFImagineEnvWrapper") -> None:
        ...
        # Get nn model instance from wrapper.
        ...
        if system_dynamics is None:
            raise RuntimeError(
                ...
                "Call `set_system_dynamics()` on the wrapper before attaching it to NNModelFKAction."
            )
        self.system_dynamics = system_dynamics

        # Dimensionalities for nn model input.
    ...
    def apply_actions(self) -> None:
        """Apply actions by rolling out the nn model and writing FK state.

        The behavior is governed by the decimation factor:

        - On steps where ``step_idx % decimation == 0``, the nn model is rolled out
        ...
        if self._counter % self.cfg.decimation == 0:
            ...
            parsed_state, *_ = self._step_nn_model(self.processed_actions)
    ...
    def _step_nn_model(
        ...
        # nn model action dimension. If not, users should provide a custom mapping.
        ...
        # Construct dynamic and action sequences for the nn model.
    ...
    def _parse_dynamic(self, dynamic: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        If the actual layout differs, users should subclass :class:`NNModelFKAction`
...
@configclass
class NNModelFKActionCfg(ActionTermCfg):
    """Configuration for :class:`NNModelFKAction`.
    ...
    decimation:
        ...
        the nn model is invoked every ``d`` calls to :meth:`apply_actions`,
        and the last predicted state is reused on intermediate steps.
    """

    class_type: type[ActionTerm] = NNModelFKAction
