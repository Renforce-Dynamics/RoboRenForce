...
if TYPE_CHECKING:
    from RoboRenForce.runners import BaseRunnerCfg
...
def add_args_group(parser: argparse.ArgumentParser):
    ...
    arg_group = parser.add_argument_group("RoboRenForce", description="Arguments for renforce rl agent.")
...
def parse_rl_cfg(task_name: str, args_cli: argparse.Namespace, rl_cfg=None) -> "BaseRunnerCfg":
    ...
    """
    if rl_cfg is None:
        rl_cfg:dict = load_cfg_from_registry(task_name, "RoboRenForce_entry_point")
...
def dump_pickle(fpath, obj):
    ...
    Some configs coming from external libraries (e.g. Isaac Lab task configs)
    may contain references that the standard ``pickle`` module cannot handle,
    which raises ``pickle.PicklingError``. In that case we log a warning and
    continue instead of crashing the training script.
    """
    try:
        ...
        print(
            f"[WARN] Skipping pickling of object {type(obj)} to '{fpath}' "
            f"due to PicklingError: {e}"
        )
