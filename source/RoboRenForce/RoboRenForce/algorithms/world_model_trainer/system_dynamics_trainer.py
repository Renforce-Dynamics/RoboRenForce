...
from RoboRenForce.buffer.direct_based.dynamic_replay_buffer import DynamicReplayBuffer
    
class SystemDynamicsTrainer:
    cfg: "SystemDynamicsTrainerCfg"
    def __init__(self, cfg: "SystemDynamicsTrainerCfg", replay_buffer, system_dynamics) -> None:
        self.cfg = cfg
    ...
    def update_system_dynamics(self):
        ...
        for (
            ...
            loss = (
                ...
                + self.cfg.system_dynamics_loss_weights["sequence"]
                * sequence_loss
                + self.cfg.system_dynamics_loss_weights["bound"] * bound_loss
                + self.cfg.system_dynamics_loss_weights["extension"]
                * extension_loss
                + self.cfg.system_dynamics_loss_weights["contact"]
                * contact_loss
                + self.cfg.system_dynamics_loss_weights["termination"]
                * termination_loss
                + self.cfg.system_dynamics_loss_weights.get("reward", 1.0)
                * reward_loss
            )
    ...
    def evaluate_system_dynamics(self):
        ...
        """
        if not hasattr(self.cfg, 'eval_trajectory_length') or self.cfg.eval_trajectory_length is None:
        ...
        if not hasattr(self.cfg, 'num_eval_trajectories') or self.cfg.num_eval_trajectories is None:
        ...
        if hasattr(self.system_dynamics, 'autoregressive_prediction'):
            dynamic_pred, extension_pred, contact_pred, termination_pred, reward_pred = \
...
class SystemDynamicsTrainerCfg(ClassTemplateBaseCfg):
    ...
    def construct_from_cfg(self, *args, **kwargs) -> "SystemDynamicsTrainer":
