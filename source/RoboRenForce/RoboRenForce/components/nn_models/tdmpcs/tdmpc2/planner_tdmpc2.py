...
from RoboRenForce.components.nn_models.tdmpcs.planner_base import PlannerBase, PlannerBaseCfg
...
class PlannerTDMPC2(PlannerBase):
    ...
    @torch.no_grad()
    def _rollout(self, nn_model, seq, cur_z, gamma, scratch):
        B, P, H, A = seq.shape
        dev = seq.device
        ...
        for t in range(H):
            ...
            next_z, r_hat, term_logits, _ = nn_model.predict_next(cur_z, a_t)
        ...
        q_term = nn_model.q_network(cur_z, last_actions).reshape(B * P)
        q_term.mul_(alive)
    ...
    @torch.no_grad()
    def _mppi_iteration(
        self, mean, std, flat_z0, nn_model, obs, scratch, gamma
    ):
        B, H, A = mean.shape
        P = self.cfg.popsize
        dev = mean.device
        ...
        returns, q_term = self._rollout(nn_model, seq, flat_z0.clone(), gamma, scratch)
    ...
    @torch.no_grad()
    def plan(self, obs, nn_model, latent=None, exec_sample=True, **kw):
        device = next(nn_model.parameters()).device

        obs = obs.to(device)
        latent = nn_model.encode(obs) if latent is None else latent.to(device)
        if latent.ndim == 3:
            latent = latent[:, -1]
        ...
        A = self.action_dim
        P = self.cfg.popsize
        gamma = getattr(nn_model, "gamma", 0.99)
        ...
        for _ in range(self.cfg.mppi_iters):
            mean, std, di = self._mppi_iteration(
                mean, std, flat_z0, nn_model, obs, scratch, gamma
            )
            diag_list.append(di)
    ...
    @staticmethod
    def _predict_q(nn_model: TDMPC2LatentDynamics, latent, action):
        return nn_model.q_network(latent, action)
