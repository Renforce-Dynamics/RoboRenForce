...
from RoboRenForce.components.encoder.vec_state_encoder import VecStateEncoderCfg
from RoboRenForce.components.decoder.continuous_vec_decoder import ContinuousVecDecoderCfg
from RoboRenForce.components.decoder.transition_vec_decoder import TransitionVecDecoderCfg

from RoboRenForce.components.actor.pi_network import PiNetwork, PiNetworkCfg
from RoboRenForce.components.critic.q_network import QNetwork, QNetworkCfg
from RoboRenForce.components.normalizer import NormalizerBaseCfg, NormalizerBase

from RoboRenForce.networks.optimizer import GroupedOptimizer, GroupedOptimizerCfg

class LatentDynamicsBase(NNModelBase):
    ...
    def save_nn_model(self, path, infos=None):
        saved_dict = {
            "infos": infos,
            "nn_model_dict": self.state_dict(),
    ...
    def load_nn_model(self, path, load_optim=False):
        loaded_dict = torch.load(path)
        self.load_state_dict(loaded_dict["nn_model_state_dict"])
        if load_optim:
...
@configclass
class LatentDynamicsBaseCfg(NNModelBaseCfg):
    """
