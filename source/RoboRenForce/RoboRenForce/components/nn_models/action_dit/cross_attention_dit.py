"""Cross-attention DiT for flow-matching action denoising.

Reimplemented from DiT4DiT to maintain state_dict compatibility.
Requires `diffusers` for Attention, FeedForward, TimestepEmbedding, Timesteps.
"""
...
class DiT(nn.Module):
    """Cross-attention DiT for action flow matching.

    State_dict compatible with DiT4DiT's cross_attention_dit.DiT.
    Removed diffusers ConfigMixin/ModelMixin to avoid config conflicts
    with RoboRenForce, but keeps identical parameter names.
    """
