...
from RoboRenForce.components.world_models.system_dynamics.system_dynamics_base import (
    SystemDynamicsBase,
...
class SystemDynamicsMLP(SystemDynamicsBase):
    ...
    - `compute_loss(state_batch, action_batch, extension_batch, contact_batch, termination_batch, reward_batch)`
      returning a 7-tuple of loss scalars compatible with `SystemDynamicsTrainer.update_system_dynamics`.
    """
    ...
    def forward(self, dynamic_seq: torch.Tensor, action_seq: torch.Tensor):
        ...
        if self.history_horizon > 1:
            ...
            dynamic_input = torch.cat([dynamic_history, action_history], dim=-1)  # [B, history_horizon, dynamic_dim + action_dim]
    ...
    def compute_loss(
        ...
        if self.history_horizon > 1:
            ...
            for i in range(Tm1):
                ...
                # Flatten
                combined = torch.cat([dynamic_window, action_window], dim=-1)  # [B, history_horizon, dynamic_dim + action_dim]
