...
from RoboRenForce.components.world_models.system_dynamics import SystemDynamicsMLPCfg
...
class MBPOOnPolicyRunner(WorldModelBasedRunner):
    ...
    def init_components(self):
        ...
        # Initialize world model components (using base class pattern)
        ...
        # Create world model trainer in runner (still using SystemDynamicsTrainer)
        self.world_model_trainer = self.cfg.world_model_trainer_cfg.construct_from_cfg(
    ...
    def save_example_from_replay(
        ...
        """Sample states from the dynamics replay buffer and roll out policy + world model.
        ...
        Args:
            ...
            out_dir: Directory to save the pickle file into. If None, defaults to
                ``<log_dir>/world_model_examples``.

        Returns:
            The path to the saved pickle file.
        """
        if predict_horizon is None:
            predict_horizon = self.cfg.world_model_trainer_cfg.dynamic_update_forecast_horizon
        ...
        if out_dir is None:
            ...
            out_dir = os.path.join(base_dir, "world_model_examples")
    ...
    def _learn(self, num_learning_iterations: int):
        ...
        for it in range(start_iter, tot_iter):
            ...
            start = time.time()
            # 2) Update world model (using base class method)
            sys_losses = self.world_model_trainer.update_system_dynamics()
            sys_losses = {f"Model/{k}": v for k, v in sys_losses.items()}

            # 3) Policy update (with or without imagination)
            if it >= start_iter + self.cfg.world_model_warmup_iterations:
                if self.imagination_learn_flag:
    ...
    def sample_rollout(self, ep_infos, obs, critic_obs, it, start_iter):
        ...
        with torch.inference_mode():
            for i in range(self.cfg.num_steps_per_env):
                ...
                if not self.cfg.frozen_wramup or it >= start_iter + self.cfg.world_model_warmup_iterations:
    ...
    def save(self, path, infos=None):
        saved_dict = {
            ...
            "world_model_optimizer_state_dict": self.world_model_trainer.model_optimizer.state_dict(),
...
@configclass
class MBPOOnPolicyRunnerCfg(WorldModelBasedRunnerCfg):
    ...
    world_model_cfg: "SystemDynamicsMLPCfg" = MISSING
    replay_buffer_cfg: DynamicReplayBufferCfg = DynamicReplayBufferCfg(buffer_size=320)
    world_model_trainer_cfg: SystemDynamicsTrainerCfg = SystemDynamicsTrainerCfg()
    world_model_normalize_cfg: NormalizerBaseCfg = NormalizerBaseCfg()  # kept for backward compatibility
    world_model_action_normalize_cfg: NormalizerBaseCfg = NormalizerBaseCfg()
    world_model_warmup_iterations: int = 0
